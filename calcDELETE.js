...
class CalculatorApp {
  constructor(widgetNode) {
    ...
    this.widgetNode
      .querySelector(".cmd-clear")
      .addEventListener("click", this.pressClear.bind(this));
    this.widgetNode
      .querySelector(".cmd-plusminus")
      .addEventListener("click", this.pressPlusMinus.bind(this));
    this.widgetNode
      .querySelector(".cmd-percent")
      .addEventListener("click", this.pressPercent.bind(this));
    this.widgetNode
      .querySelectorAll(".main-buttons .cell-row:not(.commands) .cell")
      .forEach((el) => {
    ...
    this.widgetNode
      ...
      .addEventListener("click", this.killCalculator.bind(this));
    this.widgetNode
      .querySelector(".window-minimize")
      .addEventListener("click", this.unsizeCalculator.bind(this));
    this.widgetNode
      .querySelector(".window-maximize")
      .addEventListener("click", this.cloneCalculator.bind(this));
    this.updateOutput();
  }
  ...
  pressNumber(event) {
    ...
    if (this.floatInput) {
      if (event.target.innerText === ".") {
        //toggle off decimal if double pressed
        this.floatInput = false;
        this.numberOne = String(
          parseInt(this.numberOne + String(this.numberOneFloat))
        );
        this.numberOneFloat = "";
      } else {
        //add to float instead
      ...
      if (event.target.innerText === ".") {
        ...
        this.numberOne = String(
          parseInt(this.numberOne + event.target.innerText)
        );
      }
    }
  ...
  evaluateEquation() {
    ...
    if (["+", "-"].indexOf(this.operation) >= 0) {
      evalstr +=
        10 ** Math.max(this.numberOneFloat.length, this.numberTwoFloat.length);
    } else {
      evalstr +=
        (10 **
          Math.max(this.numberOneFloat.length, this.numberTwoFloat.length)) **
        2;
    }
    console.log(evalstr);
...
var calculators = [
  new CalculatorApp(document.querySelector(".calculator-widget")),
];
